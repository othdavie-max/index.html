...
export function StoriesGrid() {
  ...
  const filtered = filter === "all" ? realTestimonials : realTestimonials.filter((t) => t.country === filter);
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="mt-8 flex flex-col items-center gap-4 rounded-2xl border border-dashed border-ink-900/15 bg-offwhite px-6 py-14 text-center">
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-gold-500/10 text-gold-500">
            <Sparkles size={22} />
          </span>
          <div>
            <p className="font-display text-lg text-ink-900">
              {filter === "all" ? "Your story could be next." : "No stories from this destination yet."}
            </p>
            <p className="mx-auto mt-2 max-w-sm text-sm text-muted">
              We only publish real, consented student journeys — book a free consultation and start yours.
            </p>
          </div>
