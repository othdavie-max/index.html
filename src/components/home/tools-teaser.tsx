...
const tools: { href: string; icon: Icon3DName; title: string; description: string; preview: string }[] = [
  {
        ...
        <RevealGroup className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
          {tools.map((tool) => (
            <RevealItem key={tool.href}>
              <Link
                href={tool.href}
                className="group flex h-full flex-col justify-between rounded-2xl border border-ink-900/8 bg-offwhite p-7 transition-all duration-300 hover:-translate-y-1 hover:scale-[1.015] hover:border-gold-500/30 hover:shadow-[0_20px_40px_-15px_rgba(11,_37,_69,0.2)]"
              >
                <div>
                  <Icon3DTile name={tool.icon} size={40} tileSize={56} />
