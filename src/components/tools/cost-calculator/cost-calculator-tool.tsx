...
const toggleFields: { key: keyof CostCalculatorInput; label: string }[] = [
...
export function CostCalculatorTool() {
  ...
  const destination = destinations.find((d) => d.code === input.country)!;
  const perYearLocal = breakdown.lineItems.reduce((sum, item) => sum + item.perYearLocal, 0);

  function update<K extends keyof CostCalculatorInput>(key: K, value: CostCalculatorInput[K]) {
    setInput((prev) => ({ ...prev, [key]: value }));
  }

  const waMessage = `Hi Baseline, I used the Cost Calculator for ${destination.name} (${input.durationYears}yr, ${input.courseType} programme). Estimated total: ${formatNaira(
