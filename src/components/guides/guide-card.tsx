...
const fieldClass = "rounded-xl border border-navy-900/12 bg-white px-4 py-3 text-sm outline-none focus:border-red-500";
...
export function GuideCard({ guide }: { guide: Guide }) {
  ...
  return (
    <>
      <div className="flex h-full flex-col rounded-2xl border border-navy-900/8 bg-offwhite p-6">
        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-navy-900 text-white">
          <FileText size={20} />
        </div>
        <h3 className="mt-4 font-display text-lg font-bold text-navy-900">{guide.title}</h3>
        ...
        <button
          ...
          className="mt-4 flex items-center justify-center gap-2 rounded-full bg-red-500 px-5 py-2.5 text-sm font-medium text-white hover:bg-red-600"
        >
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        {status === "done" && fileUrl ? (
          <div className="text-center">
            <h3 className="font-display text-lg font-bold text-navy-900">Your guide is ready</h3>
            <p className="mt-2 text-sm text-muted">It should have opened in a new tab. If not, use the link below.</p>
            <a href={fileUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-block text-sm font-medium text-red-500 underline">
              Open {guide.title}
            </a>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="flex flex-col gap-3">
            <h3 className="text-center font-display text-lg font-bold text-navy-900">{guide.title}</h3>
            ...
            <button
              ...
              className="mt-1 flex items-center justify-center gap-2 rounded-full bg-red-500 px-5 py-3 text-sm font-medium text-white hover:bg-red-600 disabled:opacity-60"
            >
