...
| `GEMINI_API_KEY` | The AI chat assistant (Gemini) — without it, the widget shows a friendly "not configured" message instead of erroring |
...
Rate-limited to 20 messages / 10 minutes per IP, 1000-character input cap, 20-turn history cap. **The streaming integration was not tested against a live model** — no `GEMINI_API_KEY` was available in this environment. The UI, error states, and the "not configured" fallback were verified; a real conversation was not.
